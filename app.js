...
async function ouvrirSessionObservation(){
  ...
  O.demarree = true;
  try{
    const { data } = await base.from('sessions').insert({
...
async function ajouterPoints(motif, points){
  const uid = await utilisateurCourant();
  if(!uid || !base) return;
  try{ await base.from('points_journal').insert({ user_id: uid, motif: motif, points: points }); }
  catch(e){}
}
...
async function cloturerComposition(note){
  ...
  try{
    ...
    await ajouterPoints('composition', note * 15);
...
document.addEventListener('click', ev=>{
  ...
  if(d.qcm !== undefined){
    ...
    if(+d.qcm === q.i) ajouterPoints('revision', 5);
  ...
  if(d.rr !== undefined){
    ...
    if(i === e.i && e.suite >= 2) { pister('ERROR_FIXED'); ajouterPoints('erreur_refermee', 20); }
    programmer(e, i===e.i);
